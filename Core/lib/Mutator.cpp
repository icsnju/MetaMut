...
int Mutator::randIndex(int n) {
  assert (n > 0);
  return getManager().randint(0, n - 1);
}

bool Mutator::randBool() {
  return getManager().randint(0, 1);
}
...
Mutator::getCompilerInstance() const {
...
std::string Mutator::generateUniqueName(
  ...
  while (true) {
    ...
    if (allIdentifiers.find(newName) !=
        allIdentifiers.end()) {
      ++suffix;
      continue;
    }
...
clang::Decl *Mutator::getMostRecentTranslationUnitDecl(
  ...
  for (clang::DeclContext *DC = FD; DC;
       DC = DC->getParent()) {
    if (DC->getParent() &&
...
clang::SourceRange Mutator::findBracesRange(
    clang::SourceLocation scrhBeginLoc, char beginBrace,
    char endBrace) {
  // Get the SourceManager from the ASTContext
  clang::SourceManager &SM =
      getASTContext().getSourceManager();
  ...
  auto beginLoc = SM.getLocForStartOfFile(locInfo.first)
                      .getLocWithOffset(foundPos);
  for (; foundPos < fileBuffer.size(); foundPos++) {
  ...
  auto endLoc = SM.getLocForStartOfFile(locInfo.first)
                    .getLocWithOffset(foundPos);
...
clang::SourceLocation Mutator::balancedSearchUntil(
  ...
  for (; foundPos < fileBuffer.size(); foundPos++) {
    ...
    default:
      if (balances.empty() &&
          untils.find(fileBuffer[foundPos]) != untils.end())
...
clang::SourceRange Mutator::findDeclaratorRange(
  ...
  unsigned begOff = endLocInfo.second,
           endOff = endLocInfo.second;
  ...
  while (nextTypeLoc) {
    clang::SourceRange range =
        nextTypeLoc.getLocalSourceRange();
    begLocInfo = SM.getDecomposedLoc(range.getBegin());
    endLocInfo = SM.getDecomposedLoc(range.getEnd());
    if (begLocInfo.second < begOff)
      begOff = begLocInfo.second;
    if (endLocInfo.second > endOff)
      endOff = endLocInfo.second;
  ...
  clang::SourceLocation begLoc =
      SM.getLocForStartOfFile(begLocInfo.first)
          .getLocWithOffset(begOff);
  clang::SourceLocation endLoc =
      SM.getLocForStartOfFile(endLocInfo.first)
          .getLocWithOffset(endOff);
...
unsigned Mutator::getSourceLocOffset(
...
clang::SourceLocation Mutator::getLocForEndOfToken(
    clang::SourceLocation loc) {
...
clang::SourceRange Mutator::getExpansionRange(
    clang::SourceRange range) {
...
clang::SourceLocation Mutator::getNextTokenLoc(
    clang::SourceLocation Loc) {
  clang::Token Tok;
  clang::SourceManager &SM = getSourceManager();
  ...
  bool Invalid = false;
  llvm::StringRef File =
      SM.getBufferData(SM.getFileID(Loc), &Invalid).str();
...
bool Mutator::replaceVarDeclarator(
  ...
  std::string NewStr =
      formatAsDecl(ty, VD->getNameAsString(), true);
...
bool Mutator::addStringBeforeFunctionDecl(
  ...
  if (clang::FunctionTemplateDecl *TmplD =
...
bool Mutator::removeParmFromFuncDecl(
  ...
  if (EndLoc.isMacroID())
    EndLoc = getSourceManager().getExpansionLoc(EndLoc);
  if (StartLoc.isInvalid() && EndLoc.isInvalid()) {
    ...
    const char *Buf =
        getSourceManager().getCharacterData(StartLoc);
    ...
    RangeSize = getRewriter().getRangeSize(
  ...
  if ((ParamPos == 0) && (TotalNumOfFuncParams == 1)) {
    return !(getRewriter().ReplaceText(
        StartLoc, RangeSize, "void"));
  }

  // The param is the last parameter
  if (ParamPos ==
      static_cast<int>(TotalNumOfFuncParams - 1)) {
    int Offset = 0;
    const char *StartBuf =
        getSourceManager().getCharacterData(StartLoc);
    ...
    return !(getRewriter().RemoveText(
        NewStartLoc, RangeSize - Offset));
  }
  ...
  return !(
      getRewriter().RemoveText(StartLoc, NewRangeSize + 1));
}

bool Mutator::removeArgFromExpr(
  ...
  const char *NextStartBuf =
      getSourceManager().getCharacterData(NextStartLoc);
  int Offset = 0;
  while (*NextStartBuf != ',') {
    NextStartBuf--;
    Offset--;
  ...
  SourceLocation NewEndLoc =
      NextStartLoc.getLocWithOffset(Offset);
  return !getRewriter().RemoveText(
      SourceRange(StartLoc, NewEndLoc));
}
...
const Expr *Mutator::getArgWrapper(
    const Expr *E, int ParamPos) {
  const CXXConstructExpr *CtorE =
      dyn_cast<CXXConstructExpr>(E);
